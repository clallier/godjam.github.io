...
let CanvasManager = function (canvas, sceneKey, options) {
  ...
  if (!this.canvas) console.log(canvasID + " does not exists")
  ...
  this.observer = new IntersectionObserver((entries, obs) =>
    this.visibilityChange(entries, obs),
  )
...
CanvasManager.prototype.handleVisibilityChange = function () {
  "use strict"
  if (this.isVisible) this.startScene()
  else this.stopScene()
}
...
CanvasManager.prototype.startScene = function () {
  "use strict"
  if (!this.canvas) {
    console.error("CanvasManager: startScene failed - canvas element is null")
    return
  }
