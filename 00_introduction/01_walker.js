...
let WalkerScene = function (options) {
  ...
  if (options.walkertype === 0) {
    ...
    this.intro(
      "Normal Distribution Walker",
      "Walker step size follows normal ditribution.",
    )
    ...
    this.intro(
      "Monte Carlo Walker",
      "Walker step size follows Monte Carlo method.",
    )
  ...
  this.addListener(new MouseEvtListener(this, p => this.walker.mouseEvent(p)))
}
